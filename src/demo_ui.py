...
def retrieve_for_case(
    ...
    if not messages:
        ...
        session = next((s for s in (user or {}).get("sessions", []) if s.get("thread_id") == thread_id), None)
        messages = (session or {}).get("messages", [])
    for msg in messages or []:
        short_mem.add(msg["role"], msg["content"])
    for msg in extra_messages or []:
        short_mem.add(msg.get("role", "user"), msg.get("content", ""))

    st_text = short_mem.render()

    wanted = case.get("retrieve_layers")
    if not wanted:
        if layer == "mixed":
            wanted = ["long_term", "semantic"]
        elif layer in ("short_term", "long_term", "episodic", "semantic"):
            wanted = [layer]
        else:
            wanted = ["long_term"]

    layers: dict[str, str] = {
        "short_term": st_text if ("short_term" in wanted or layer == "short_term") else "",
        "long_term": "",
        "episodic": "",
        "semantic": "",
    }
